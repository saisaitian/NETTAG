...
def sparse_or_dense_dropout(x, p=0.5, training=True, rand_seed = 1234):
...
class PolyGraphConvolution(nn.Module):
    ...
    def forward(self, x, poly_ls):
        ...
        for idx in range(len(poly_ls)):
            out += to_sparse_tensor(poly_ls[idx]) @ (x @ self.weight[idx,:,:])
        return out


class PolyGCN(nn.Module):

    def __init__(self, input_dim, hidden_dims, output_dim, adj_pow, dropout, rand_seed, batch_norm=False):
        super().__init__()
    ...
    def forward(self, x, poly_ls):
        for idx, poly_gcn in enumerate(self.layers):
            if self.dropout != 0:
                x = sparse_or_dense_dropout(x, p=self.dropout, training=self.training, rand_seed = self.rand_seed)
